...
# ── общий LLM-фолбэк (OpenAI → Gemini → Groq) ─────────────────────────────
from gpt import gpt_complete
...
# ───── GPT-helpers (через общую обёртку) ──────────────────────────────────
async def gpt_short(date: str, phase: str) -> List[str]:
    """3 одно-строчных совета с emoji или fallback"""
    prompt = (
        f"Дата {date}, фаза {phase}. "
        f"Действуй как профессиональный астролог, который знает, как Луна влияет на человека. "
        f"Дай 3 очень кратких совета, каждый отдельной строкой с эмодзи: "
        f"💼 (работа), ⛔ (отложить), 🪄 (ритуал). "
        f"Лимит ~12 слов на строку, без лишних пояснений."
    )
    try:
        text = await asyncio.to_thread(gpt_complete, prompt, 0.65, 200)
        lines = [l.strip() for l in (text or "").splitlines() if l.strip()][:3]
        if len(lines) == 3:
            return lines
    except Exception:
        pass
...
async def gpt_long(name: str, month: str) -> str:
    ...
    prompt = (
        ...
        f"Дай 1–2 коротких предложения, описывающих энергетику периода. "
        f"Тон экспертный, вдохновляющий, без лишней воды."
    )
    try:
        text = await asyncio.to_thread(gpt_complete, prompt, 0.7, 300)
        txt = (text or "").strip()
        if txt:
            return txt
    except Exception:
