...
from gpt import gpt_complete  # ← используем общую обёртку вместо прямого SDK
...
async def gpt_short(date: str, phase: str) -> List[str]:
    ...
    try:
        txt = gpt_complete(prompt=prompt, system=None, temperature=0.65, max_tokens=300)
...
async def gpt_long(name: str, month: str) -> str:
    ...
    try:
        txt = gpt_complete(prompt=prompt, system=None, temperature=0.7, max_tokens=400)
        if txt:
            return txt.strip()
    except Exception:
        pass
...
async def generate(year: int, month: int) -> Dict[str,Any]:
    ...
    while d <= last:
        ...
        cal[d.to_date_string()] = {
            "phase_name"     : name,
            "phase"          : f"{emoji} {name} , {sign}",  # сохранили прежний формат
            "percent"        : illum,
            "sign"           : sign,
