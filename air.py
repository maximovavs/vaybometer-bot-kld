...
• get_sst() — текущая температура поверхности моря (SST) для любых lat, lon
...
from utils import _get  # ваша утилита для HTTP-запросов

# ────────── Константы ───────────────────────────────────────────────
# Для качества воздуха (IQAir) используются глобальные координаты Limassol
LAT_AIR, LON_AIR = 34.707, 33.022  
...
def _src_iqair() -> Optional[Dict[str, Any]]:
    if not AIR_KEY:
        return None
    j = _get("https://api.airvisual.com/v2/nearest_city", lat=LAT_AIR, lon=LON_AIR, key=AIR_KEY)
...
def _src_openmeteo() -> Optional[Dict[str, Any]]:
    j = _get(
        ...
        latitude=LAT_AIR, longitude=LON_AIR,
...
def get_sst(lat: float, lon: float) -> Optional[float]:
    ...
    Используем Open-Meteo с параметром sea_surface_temperature.

    Пример запроса:
      https://api.open-meteo.com/v1/forecast?
        latitude={lat}&longitude={lon}&hourly=sea_surface_temperature&timezone=UTC

    Возвращаем первое значение hourly["sea_surface_temperature"][0], либо None.
    """
    try:
        j = _get(
            "https://api.open-meteo.com/v1/forecast",
...
if __name__ == "__main__":
    ...
    print("SST (пример для Калининграда):", get_sst(54.710426, 20.452214))
    print("Kp:", get_kp())