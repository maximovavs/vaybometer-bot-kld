...
# Примерные координаты для расчёта погоды:
SEA_CITIES_ORDERED = [
...
# Список «не-морских» городов (берём топ-3 тёплых и топ-3 холодных динамически)
# Для упрощения просто передаём весь список, а внутри main_common возьмёт по температуре
OTHER_CITIES_ALL = [
