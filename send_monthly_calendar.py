...
• если данные «ровные» (все дни в одну фазу/пустые) — включает аварийное
  ...
  используются приоритетно, но если они повторяются/короткие/слишком общие —
  подменяются фолбэком для сегмента;
...
def _norm_text(s: str) -> str:
    """Нормализуем текст для сравнения/дедупликации."""
    return " ".join((s or "").split()).strip().lower()


def _is_generic_monthwide(s: str) -> bool:
    """Эвристика: фраза про «весь/целый месяц», «в течение месяца», «с 1 по 30…» и т.п."""
    if not s:
        return False
    return bool(GENERIC_RE.search(s))
...
def build_phase_blocks_with_fallback(data: Dict[str, Any]) -> str:
    ...
    выбираем majority-эмодзи и ДЕДУПЛИРУЕМ одинаковые/общие/короткие фразы Gemini.
    """
    ...
    seen_norms: set[str] = set()  # уже использованные тексты Gemini (нормализованные)

    for idx, (si, ei) in enumerate(segs):
        span = _format_span(days, si, ei)
        emoji = _major_emoji_for_segment(
            data, days, si, ei,
            FALLBACK_EMOJI[idx] if idx < len(FALLBACK_EMOJI) else "🌙"
        )
        ...
        # Кандидаты Gemini внутри сегмента — собираем все и ищем «лучший»
        candidates: List[str] = []
        for k in range(si, ei + 1):
        ...
        for cand in candidates:
            ...
            if len(cand) >= MIN_DESC_LEN and not _is_generic_monthwide(cand) and norm not in seen_norms:
                chosen = cand
                seen_norms.add(norm)
                break

        # Если не нашли хороший уникальный текст — мягкий фолбэк
        if not chosen:
            chosen = FALLBACK_TEXTS[idx] if idx < len(FALLBACK_TEXTS) else ""

        desc = html.escape(chosen) if chosen else ""
        header = f"<b>{emoji} {span}</b>"
        if signs_str:
            header += f" <i>({signs_str})</i>"
...
def build_message(data: Dict[str, Any]) -> str:
    ...
    2) Блок фаз (с аварийным режимом при «коллапсе» + дедуп/анти-генерик)
